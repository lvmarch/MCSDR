...
class Uniformer(nn.Module):
    def __init__(self, depth=[5, 8, 20, 7], num_classes=1, img_size=224, in_chans=3, embed_dim=[64, 128, 320, 512],
        ...
        self.hierarchical_output_dim = 512
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        x = self.pre_logits(x)
        features_list.append(x.flatten(2).mean(-1))  # f4
...
def uniformer_small(pretrained=False, **kwargs):
    model = Uniformer(
        ...
        head_dim=64, drop_rate=0.1, **kwargs)

    model.output_dim = model.hierarchical_output_dim
